...
public class ProxyPageModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        var uri = IsExternalUri(Request.Path)
            ? ParseExternalUri(Request.Path)
            : $"{_config.RemoteUri}{Request.Path}";
        ...
        if (response is ErrorResponse error)
            switch (error.Status)
            {
                ...
                case Opal.Response.StatusCode.ProxyRequestRefused:
                    return StatusCode(StatusCodes.Status500InternalServerError, response.ToString() ?? string.Empty);
